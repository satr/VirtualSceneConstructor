...
namespace VirtualScene.DataComponents.Common.DataAdapters
{
    ...
    {
        private readonly IDictionary<Type, object> _dataAdapters = new Dictionary<Type, object>();
        ...
        {
            _dataAdapters.Add(typeof(IStage), new StageFileSystemDataAdapter());
        }
        ...
        public virtual IDataAdapter<T> GetFileSystemDataAdapter<T>()
        {
            ...
            if (_dataAdapters.ContainsKey(dataAdapterType))
                return (IDataAdapter<T>) _dataAdapters[dataAdapterType];
